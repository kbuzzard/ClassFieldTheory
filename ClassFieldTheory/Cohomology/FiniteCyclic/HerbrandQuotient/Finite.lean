...
lemma herbrandQuotient_of_finite [Fintype G] [Finite A] : ρ.herbrandQuotient = 1 := by
  /-
  Consider the linear maps `oneSubGen` and `norm : M → M` defined to be multiplication by
  `1 - gen G` and norm respectively. The kernel of `oneSubGen` is the submodule of `G`-invariants,
  and the cokernel of `oneSubGen` is the quotient module of coinvariants. We therefore have
  (for Tate groups):
...
lemma herbrandQuotient_of_finite [Finite G] (M : Rep R G) [Finite M] : M.herbrandQuotient = 1 := by
  cases nonempty_fintype G; simpa using M.ρ.herbrandQuotient_of_finite
