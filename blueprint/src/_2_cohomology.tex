...
\begin{lemma} \label{lem:Hn unit}
	\lean{Rep.instTrivialCohomologyOfSubsingleton}
	\leanok
...
\begin{lemma} \label{lem:cochainsFunctor exact}
	\lean{groupCohomology.map_cochainsFunctor_shortExact}
	\mathlibok
...
\begin{lemma} \label{lem:homology 1 trivial}
	\lean{groupHomology.H1AddEquivOfIsTrivial}
	\uses{def:group homology}
	\mathlibok
	\leanok
...
\begin{lemma}	\label{lem:Tate cohomology is cohomology or homology}
  \lean{TateCohomology.isoGroupCohomology,
    TateCohomology.isoGroupHomology,
		TateCohomology.zeroIso,
		TateCohomology.zeroIsoOfIsTrivial,
		TateCohomology.negOneIso,
		TateCohomology.negOneIsoOfIsTrivial}
...
\begin{definition}	\label{def:Tate long exact sequence}
  \lean{TateCohomology.preservesFiniteLimits_tateComplexFunctor,
    TateCohomology.preservesFiniteColimits_tateComplexFunctor,
		TateCohomology.δ}
	\uses{def:Tate cohomology,
...
\begin{lemma}	\label{lem:coind₁ trivial cohomology}
	\lean{Rep.trivialCohomology_coind₁}
...
\begin{lemma}	\label{lem:induced trivial Tate}
	\lean{Rep.trivialTateCohomology_coind₁,
...
\begin{definition} \label{def:up}
	\lean{Representation.coind₁'_ι,
		...
		Rep.dimensionShift.upShortComplex,
...
\begin{corollary}	\label{cor:up iso}
	\lean{Rep.dimensionShift.δ_up_zero_epi,
		Rep.dimensionShift.δ_up_isIso,
		Rep.dimensionShift.δUpIso,
		Rep.dimensionShift.epi_δ_up_zero_res,
		Rep.dimensionShift.isIso_δ_up_res,
		Rep.dimensionShift.δUpResIso,
		Rep.dimensionShift.δUpNatIso}
	\uses{def:up,
...
\begin{definition} \label{def:down}
	\lean{Representation.ind₁'_π,
		...
		Rep.dimensionShift.shortExact_downSES_res
	}
	\uses{def:ind'}
	\leanok
...
\begin{corollary}	\label{cor:Tate up down isos}
	\lean{groupCohomology.instIsIso_shortExact_upSES,
		groupCohomology.instIsIso_shortExact_downSES,
		groupCohomology.δUpIsoTate,
		groupCohomology.δDownIsoTate
	}
	\uses{
		def:up,
		def:down,
...
\begin{definition} \label{def:corestriction}
	...
	be a set of representatives for the left cosets $r_i S$.
	For any representation $M$ of $G$ there is a linear map $N_{G/S} : M^S \to M^G$
	defined by
	\[
		N_{G/S}(m) = \sum_i r_i \bullet m.
	\]
	This map does not depend on the choice of coset representatives.
	...
	\begin{itemize}
		...
		\[
			\begin{matrix}
				...
				\downarrow &&\downarrow \\
				H^n(G,\coind_1'(M)) & \to & H^n(G,\up(M)) & \to &H^{n+1}(G,M) & \to & 0
			\end{matrix}.
		\]
		...
		\[
			\begin{matrix}
				...
				\downarrow &&\downarrow \\
				H^n(G,\up(M)) & \to &H^{n+1}(G,M)
			\end{matrix}.
		\]
	\end{itemize}
	The map $\cor^{n} : H^n(S , -) \to H^n(G,-)$ is a morphism of functors.
\end{definition}
...
\begin{proof}
	...
	the kernel.
	Conversely suppose $h : G \to_0 M$ satisfies $\sum_{i=0}^{n-1} h(\gen^i) = 0$.
	Then we have
	\begin{align*}
		h
...
\begin{corollary} \label{cor:periodic cohomology}
	\uses{cor:Tate up down isos,
		def:up iso down}
	\lean{Rep.periodicCohomology, periodicTateCohomology}
  \leanok
...
\begin{lemma} \label{lem:H2 cyclic Z}
	...
	\lean{groupCohomology.evenTrivialInt, groupCohomology.isZero_odd_trivial_of_isAddTorsionFree}
	\leanok
...
\begin{lemma} \label{lem:local inv iso}
	\lean{carry, carryCocycle, localInv, localInvIso, localInv_symm_apply}
	\leanok
...
\begin{proof}
	...
	$\tilde i + \tilde j - (\widetilde{i + j})$ for $\tilde\cdot : \Z/n\Z \to \Z$ a section.
...
\begin{definition}
  \label{def:herbrand quotient}
  \uses{def:Tate cohomology}
	\lean{Representation.herbrandQuotient, Rep.herbrandQuotient}
  \leanok
...
\begin{example}
  \label{eg:herbrand Z}
	\uses{lem:H2 cyclic Z}
  \lean{Representation.herbrandQuotient_trivial_int_eq_card,
...
\begin{lemma}
  \label{lem:herbrand finite}
	\uses{lem:Tate cohomology is cohomology or homology}
  \lean{Representation.herbrandQuotient_of_finite, Rep.herbrandQuotient_of_finite}
  \leanok
...
\begin{lemma}
  \label{lem:herbrand ses}
  \uses{def:herbrand quotient}
  \lean{Rep.herbrandQuotient_ne_zero_of_shortExact₁, Rep.herbrandQuotient_ne_zero_of_shortExact₂,
    Rep.herbrandQuotient_ne_zero_of_shortExact₃, Rep.herbrandQuotient_eq_of_shortExact}
  \leanok
...
\begin{proof}
  ...
  \leanok
	...
	Also, by periodicity (\ref{cor:periodic cohomology}),
  the long exact sequence reduces to an exact hexagon:
	\[
		\begin{matrix}
			&& H^0_{\Tate}(G,B) \\
			&\nearrow && \searrow \\
...
$G$ and all $n > 0$, the cohomology groups $H^{n}(S,M)$ are zero. We begin with a theorem
which will immediately be made redundant by the theorem after.

\begin{theorem} \label{thm:triviality criterion solvable}
	\lean{groupCohomology.trivialCohomology_of_even_of_odd_of_solvable}
	\leanok
	Let $M$ be a representation of a finite solvable group $G$ (note that the solvability
	condition will be removed in the next theorem).
...
\begin{definition}
  ...
  \lean{Rep.split.FiniteClassFormation}
  \leanok
	In this section $G$ is a finite group and $R$ is a commutative ring. If $M$ is a representation
	of $G$ over $R$ then we shall call $M$ (or more precisely the triple $(R,G,M)$)
	a \emph{finite class formation} if:
	\begin{itemize}
		\item
	...
	\end{itemize}
	If $M$ is a finite class formation then a generator $\sigma$ for $H^2(G,M)$ (by which we
	mean the image of 1 under some isomorphism $R/|G|R\equiv H^2(G,M)$) is called
...
\[
	\reciprocity: G^{\ab} \otimes R \cong H^0_{\Tate}(G,M).
\]
The reciprocity isomorphism depends on the choice of a fundamental class. In our application
to local class field theory we will isolate a ``canonical'' fundamental class, called ``the''
fundamental class.

\begin{remark}
	\begin{itemize}
		\item Given $G$ and $R$, there can be several nonisomorphic $M$ which are
  		finite class formations. For example if $M$ is a finite class formation then so is
	  	$M\oplus N$ where $N$ is any cohomologically trivial module.
	  \item I know of no example in class field theory where $R$ is any ring other than $\Z$.
	\end{itemize}
\end{remark}

\begin{example}
	If $G$ is a finite cyclic group then the trivial representation $\Z$
	(or more precisely the triple $(\Z,G,\Z)$) is a finite class formation.
	This follows from \ref{lem:H2 cyclic Z}.
	The cocycle $\sigma_G$ defined in \ref{lem:local inv iso} (which depends on
	a choice of generator for $G$) is a fundamental class. Note that it is unsurprising
	that the cocycle depends on a choice of generator, as the
	reciprocity isomorphism gives an isomorphism $G\cong \Z/|G|\Z.$
\end{example}
...
\end{proof}
\begin{remark} In the case $R=\Z$ and $I\not=0$, which is the only case
	we shall use in applications, the lemma can just be proved by counting.
\end{remark}
...
\begin{remark} Again when $R=\Z$ the above proof can be made much simpler.
\end{remark}

We issue a warning: the previous lemma shows that the restriction of a fundamental class to
a subgroup is a fundamental class. Later on we shall be talking about \emph{the} fundamental class
in local class field theory, as opposed to \emph{a} fundamental class, and
lemma~\ref{lem:restriction fundamental class generates} will not be good enough; we will
need to check that \emph{the} fundamental class restricts to \emph{the} fundamental class.

Say $(R,G,M)$ is a finite class formation. We now begin the construction of the reciprocity
isomorphism $G^{\ab}\otimes R\cong H^0_{\Tate}(G,M)$ from a fundamental class.
...
\begin{proof}
	...
	Hence $H^2(S,\Split(\sigma')) \cong 0$.
...
\begin{definition}
  \label{def:reciprocity iso}
	\uses{thm:splitting module trivial,
		lem:Tate -1 aug,
		thm:trivial cohomology implies trivial Tate}
	\lean{Rep.split.reciprocityIso}
  The theorem implies that we have isomorphisms for all $n\in \Z$ (which depend on $\sigma$):
	\[
...
\begin{lemma} \label{lem:reciprocity formula}
	...
	\[
		\reciprocity(gG' \otimes 1) \equiv  \sum_{x \in G} \sigma'(x,g) \bmod N_G M.
	\]
...
\begin{proof}
	...
	\]
	then we will add the following term to $\reciprocity(g)$:
	\[
		\sum_{x \in G} (x \bullet \tau(g) - \tau(xg) + \tau(x)).
	\]
	The second and third terms cancel each other out after reindexing, so we are
...
\begin{proof}
	...
	is a fundamental class is \ref{lem:restriction fundamental class generates}.
...
% \begin{lemma}
% 	Let $(R,G,M)$ be a finite class formation and let $S$ be a normal subgroup of $G$.
% 	Then the reciprocity map $G^{\ab} \cong M^G / N_G(M)$ maps elements of $SG' / G'$ to
